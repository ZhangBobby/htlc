...
contract HashedTimeLockERC721 {
    ...
    event HTLCERC721Withdraw(string indexed htlcId);
    event HTLCERC721Refund(string indexed htlcId);
   ...
   //fisco bcos链时间戳精确到毫秒
    ...
    /**
     * @dev 创建一个新的erc721 HTLC
  
     */
    function newERC721HTLC(
        address _receiver,
        string _hashlock,
        uint256 _timelock,
        ...
        returns (bytes32 htlcId)
    {
		...
		if(TokenErc721(_tokenContract).getApproved(_tokenId) != address(this))
			revert("The HTLC must have been designated an approved spender for the tokenId");
        ...
        // 先转到合约账户进行锁定
        if (!TokenErc721(_tokenContract).transferFrom(msg.sender, address(this), _tokenId))
        ...
        returns (bool)
    {
        ...
        c.withdrawn = true;
        TokenErc721(c.tokenContract).transferFrom(address(this), c.receiver, c.tokenId);
        emit HTLCERC721Withdraw(_htlcId);
        return true;
    }
        ...
        returns (bool)
    {
        LockHTLCInfo storage c = lockHTLCInfos[_htlcId];
        c.refunded = true;
        TokenErc721(c.tokenContract).transferFrom(address(this), c.sender, c.tokenId);
        emit HTLCERC721Refund(_htlcId);
        return true;
    }
    ...
    //只读方法，判断htlcId是否已存在
    function haveHTLCId(string _htlcId)
        view
        returns (bool exists)
    {
